...
repositories {
    ...
    maven {
        url = uri("https://sopra-gitlab.cs.tu-dortmund.de/api/v4/projects/1482/packages/maven")
        credentials(HttpHeaderCredentials::class) {
            name = "Private-Token"
            value = "glpat-dBgL4rALTBEKATdqz5iC"
        }
        authentication {
...
dependencies {
    ...
    implementation(group = "tools.aqua", name = "bgw-gui", version = "0.8.1-nightly-8-8e4d7c3-SNAPSHOT")
...
tasks.distZip {
    ...
    into(""){
        from(".")
        include("Ingenious_How_to_Play.pdf")
    }
}
...
tasks.jacocoTestReport {
    ...
    classDirectories.setFrom(files(classDirectories.files.map {
        fileTree(it) {
            exclude(listOf("view/**","entity/**", "service/AiService*.*","service/AiRandomService*.*", "Main*.*"))
        }
    }))
}
