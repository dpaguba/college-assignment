"""Abfragen über eine Tabelle: Aufgabe 1 des siebten Tutoriums."""
...
def all_sellers(connection):
    """Teil a: alle Datensätze der Tabelle verkaeufer."""
    return connection.execute("SELECT * FROM verkaeufer").fetchall()


def warehouse_places(connection):
    """Teil b: alle Lagerorte, alphabetisch sortiert."""
    return [row[0] for row in connection.execute(
        "SELECT ort FROM lager ORDER BY ort").fetchall()]


def articles_by_name(connection):
    """Teil c: alle Artikel mit Bezeichnung, aufsteigend sortiert."""
    return connection.execute(
        "SELECT anr, abez FROM artikel ORDER BY abez").fetchall()


def expensive_articles(connection, floor=100.0):
    """Teil d: Artikel über einer Preisgrenze, absteigend nach Nummer."""
    return connection.execute(
        "SELECT anr, abez FROM artikel WHERE preis > ? ORDER BY anr DESC",
        (floor,)).fetchall()


def distinct_customer_names(connection):
    """Teil e: Vor- und Nachnamen ohne Mehrfachnennungen.

    Hier entscheidet DISTINCT über die Antwort. Zwei Kunden heissen Otto
    Bachmann und stehen mit verschiedenen Kundennummern in der Tabelle;
    ohne DISTINCT erscheint der Name zweimal, mit DISTINCT einmal. Welche
    Antwort richtig ist, sagt die Frage: sie will Namen, nicht Kunden.
    """
    return connection.execute(
        "SELECT DISTINCT name, vorname FROM kunde ORDER BY name, "
        "vorname").fetchall()


def customers_in(connection, place):
    """Teil f: Kunden eines Ortes, nach Name sortiert."""
    return connection.execute(
        "SELECT name, vorname FROM kunde WHERE ort = ? ORDER BY name",
        (place,)).fetchall()


def customers_in_either(connection, first, second):
    """Teil g: Kunden aus zwei Orten.

    Die Frage lautet «welche Kunden wohnen in Dortmund und Bochum» und
    meint das Gegenteil dessen, was sie wörtlich sagt: kein Kunde wohnt an
    zwei Orten zugleich, also wäre die Menge mit UND leer. Gesucht ist die
    Vereinigung, und in SQL heisst sie OR oder IN.
    """
    return connection.execute(
        "SELECT name, vorname, ort FROM kunde WHERE ort IN (?, ?) "
        "ORDER BY ort, name", (first, second)).fetchall()


def the_and_trap(connection, first, second):
    """Zeigt, was die wörtliche Lesart der Frage ergibt.

    Returns:
        Abbildung mit beiden Ergebnisgrössen.
    """
    both = connection.execute(
        "SELECT name FROM kunde WHERE ort = ? AND ort = ?",
        (first, second)).fetchall()
    either = connection.execute(
        "SELECT name FROM kunde WHERE ort = ? OR ort = ?",
        (first, second)).fetchall()
    return {"with and": len(both), "with or": len(either),
            "why": "eine Spalte kann in einer Zeile nur einen Wert haben"}
